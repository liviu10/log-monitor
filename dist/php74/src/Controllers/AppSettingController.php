...
class AppSettingController extends BaseController
{
    ...
    public function store(array $postData): void
    {
        ...
        if (isset($postData['settings']) && is_array($postData['settings'])) {
            ...
            foreach ($postData['settings'] as $index => $item) {
                ...
                try {
                    ...
                    LogViaStream::send(LogLevel::ERROR, 'Bulk setting storage exception', [
                        'location' => __METHOD__,
                        'line' => __LINE__,
        ...
        try {
            ...
            LogViaStream::send(LogLevel::ERROR, 'Single setting storage exception', [
                'location' => __METHOD__,
                'line' => __LINE__,
    ...
    public function update(array $postData): void
    {
        ...
        if (isset($postData['settings']) && is_array($postData['settings'])) {
            ...
            foreach ($postData['settings'] as $index => $item) {
                ...
                try {
                    ...
                    LogViaStream::send(LogLevel::ERROR, 'Bulk setting update exception', [
                        'location' => __METHOD__,
                        'line' => __LINE__,
                        'exception_message' => $e->getMessage(),
                        'exception_file' => $e->getFile(),
        ...
        try {
            ...
            LogViaStream::send(LogLevel::ERROR, 'Single setting update exception', [
                'location' => __METHOD__,
                'line' => __LINE__,
                'exception_message' => $throwable->getMessage(),
                'exception_file' => $throwable->getFile(),
    ...
    public function delete(array $postData): void
    {
        ...
        try {
            ...
            LogViaStream::send(LogLevel::ERROR, 'Setting deletion exception', [
                'location' => __METHOD__,
                'line' => __LINE__,
                'exception_message' => $throwable->getMessage(),
                'exception_file' => $throwable->getFile(),
