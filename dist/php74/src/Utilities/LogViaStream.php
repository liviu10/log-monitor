...
final class LogViaStream
{
     ...
     * @var int
     */
    private const MAX_LOGS_PER_REQUEST = 30;
      // Maxim loguri transmise de un singur script/apel
    /**
     * @var int
     */
    private const MAX_LOGS_PER_MINUTE = 300;  // Maxim loguri acceptate global de pe tot serverul intr-un minut
        ...
        register_shutdown_function(static function (): void {
            ...
            if ($hasFatalError || $hasOrphanedBuffer) {
                ...
                if ($hasFatalError) {
                    ...
                    switch (true) {
                        case strpos($message, 'Allowed memory size') !== false:
                            $type = 'Fatal Out Of Memory (RAM Exceeded)';
                            break;
                        case strpos($message, 'Maximum execution time') !== false:
                            $type = 'Fatal Execution Timeout';
                            break;
                        default:
    ...
    private static function handleException(Throwable $exception): void
    {
        ...
        } elseif ($exception instanceof \PDOException || strpos(get_class($exception), 'OCI') !== false) {
    ...
    public static function send(string $level, string $message, array $context = []): bool
    {
        ...
        try {
            ...
            $extendedContext = array_merge([
                ...
                'query_params' => $_GET === [] ? null : self::sanitizeData($_GET),
                'post_data'    => $_POST === [] ? null : self::sanitizeData($_POST),
    ...
    {
        if (stripos($message, 'ora-01017') !== false || stripos($message, 'logon denied') !== false) {
        ...
        if (strpos($message, 'Database Failure') === false) {
